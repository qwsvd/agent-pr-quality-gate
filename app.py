...
st.write(
"This dashboard summarizes a quality gate workflow for reviewing AI coding agent outputs before they are submitted to real open-source GitHub projects."
)
...
if "final_outcome" in quality_results.columns:
    generated_patches = (
        quality_results["final_outcome"] == "patch_generated_and_accepted_for_future_pr"
    ).sum()

if "human_acceptability" in quality_results.columns:
    accepted_cases = (
        quality_results["human_acceptability"] == "acceptable"
    ).sum()
...
if quality_results.empty:
    ...
    for index, row in quality_results.iterrows():
...
st.write("No pull request has been created yet.")
